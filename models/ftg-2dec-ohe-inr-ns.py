...
def define_models(dim_o, dim_q, n_units):
	...
	decoder_inputs = concatenate([dec_in_o, dec_in_q])
	...
	decoder_inputs_2 = concatenate([dec_in_o_2, dec_in_q_2])
	...
	model = Model([enc_in_o, enc_in_q, dec_in_o, dec_in_q], [out_o, out_q])
	model_2 = Model([enc_in_o, enc_in_q, dec_in_o_2, dec_in_q_2], [out_o_2, out_q_2])
	
	# define inference encoder
	encoder_model = Model([enc_in_o, enc_in_q], encoder_states)
	...
	decoder_model = Model([dec_in_o, dec_in_q] + decoder_states_inputs, [out_o, out_q] + decoder_states)
	decoder_model_2 = Model([dec_in_o_2, dec_in_q_2] + decoder_states_inputs_2, [out_o_2, out_q_2] + decoder_states_2)
...
def predict_sequence(infenc, infdec, src_o, src_q, n_steps, cardinality_o, cardinality_q):
	# encode
	state = infenc.predict([src_o, src_q])
	...
	for t in range(n_steps):
		...
		#print(state[0].shape)
		o, q, h, c = infdec.predict([target_o, target_q] + state)
		#print(a)
		# store prediction
		output.append(o[0,0,:])
		output.append(q[0,0,:])
...
def generatorex(features1, features2, seq_length, ft_o, ft_q, m_o, m_q, batch_size):
    ...
    while True:
        ...
        yield [batch_features1, batch_features2, batch_feat_pad1, batch_feat_pad2], [batch_features1, batch_features2]
...
n_patterns = [len(dataX1_q), len(dataX1_q_2)]
...
# define models
train, train_2, infenc, infdec, infdec_2 = define_models(n_features_o, n_features_q, 128)
train.compile(optimizer='adam', loss={'tr_out_o': 'categorical_crossentropy', 'tr_out_q': 'categorical_crossentropy'},
 metrics={'tr_out_o': 'accuracy', 'tr_out_q': 'accuracy'})
train_2.compile(optimizer='adam', loss={'tr_out_o_2': 'categorical_crossentropy', 'tr_out_q_2': 'categorical_crossentropy'},
 metrics={'tr_out_o_2': 'accuracy', 'tr_out_q_2': 'accuracy'})
...
for i in range(epochs_c):
    ...
    history1 = train.fit(generatorex(dataX1_o, dataX1_q, seq_length, n_features_o, n_features_q, max_o, max_q, batch_size=540), steps_per_epoch= (dtlngth[0]-split_i[0]) // 540, verbose=2)
    print ("Epoch", str(i+1)+"_b")
    history2 = train_2.fit(generatorex(dataX1_o_2, dataX1_q_2, seq_length, n_features_o, n_features_q, max_o, max_q, batch_size=540), steps_per_epoch= (dtlngth[1]-split_i[1]) // 540, verbose=2)
