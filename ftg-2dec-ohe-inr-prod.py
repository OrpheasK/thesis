...
with open('/data/spock/data1/users/el13102/midi21txt/Rock_Cleansed/998/3/TREEHIU128F425CA33.txt', 'r') as f: 
	reader = csv.reader(f)
...
train = load_model("/data/spock/data1/users/el13102/weight/Rock-Tetr inR/ohe 20x2-30-540/train.h5")
train_2 = load_model("/data/spock/data1/users/el13102/weight/Rock-Tetr inR/ohe 20x2-30-540/train_2.h5")

encoder_inputs_o = train.input[0]   # input_1 concat
encoder_inputs_q = train.input[1]
#encoder_inputs_p = train.input[2]
encoder_outputs, state_h_enc, state_c_enc = train.layers[6].output   # lstm_1
encoder_states = [state_h_enc, state_c_enc]

infenc = Model([encoder_inputs_o, encoder_inputs_q], encoder_states)

decoder_inputs_o = train.input[2]   # input_2 concat
decoder_inputs_q = train.input[3]
#decoder_inputs_p = train.input[5]
decoder_inputs = train.layers[5].output
...
decoder_lstm = train.layers[7]
decoder_outputs, state_h_dec, state_c_dec = decoder_lstm(decoder_inputs, initial_state=decoder_states_inputs)
decoder_states = [state_h_dec, state_c_dec]
decoder_dense_o = train.layers[-2]
decoder_dense_q = train.layers[-1]
...
infdec = Model([decoder_inputs_o, decoder_inputs_q] + decoder_states_inputs, [decoder_outputs_o, decoder_outputs_q] + decoder_states)

decoder_inputs_o_2 = train_2.input[2]   # input_2 concat
decoder_inputs_q_2 = train_2.input[3] 
#decoder_inputs_p_2 = train_2.input[5] 
decoder_inputs_2 = train_2.layers[5].output
...
decoder_lstm_2 = train_2.layers[7]
...
decoder_dense_o_2 = train_2.layers[-2]
decoder_dense_q_2 = train_2.layers[-1]
...
infdec_2 = Model([decoder_inputs_o_2, decoder_inputs_q_2] + decoder_states_inputs_2, [decoder_outputs_o_2, decoder_outputs_q_2] + decoder_states_2)
...
for i in range(0, n_patterns, seq_length):
    ...
    X1_q = np.reshape(to_categorical([ql_to_int[ql[0]] for ql in dataX1_q[i]], num_classes=n_features_q), (1, seq_length, n_features_q))
    #X1_p = np.reshape(to_categorical(dataX1_p[i], num_classes=n_features_p), (1, seq_length, n_features_p))
    target = predict_sequence(infenc, infdec_2, X1_o, X1_q, n_steps_out, n_features_o, n_features_q)
    for j in range(seq_length):
        re_sub_list = [int_to_ql[one_hot_decode([target[2*j]])[0]], int_to_ql[one_hot_decode([target[2*j+1]])[0]]]
        re_stream_list = re_stream_list + [re_sub_list]
...
with open("/data/spock/data1/users/el13102/TREEHIU128F425CA33inR_rtt.txt", 'w') as outcsv:   
